...
void anaL1Bin::anaWFfillRootFile(TString fileAnaSetups,TString outRootFileF){
  ...
  TString sigPolarity[USBWCnChannels];
  ...
  for(i = 0;i<USBWCnChannels;i++){
    ...
    indataSetup>>sigPolarity[i];
  }
  /////////////////////////////////////////
  cout<<setw(20)<<"channel"<<setw(20)<<"nPointBaseLine"<<setw(20)<<"sigConstTh"<<setw(20)<<"chargeWinL"<<setw(20)<<"chargeWinR"<<setw(20)<<"sigPolarity"<<endl;
  for(i = 0;i<USBWCnChannels;i++){
    cout<<setw(20)<<i
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	ch_T_usb[i] = i*SamplingPeriod_usbwc/1000.0;
      ...
      //wf = new waveformSimple( ch_T_usb, ch_A_usb, USBWCnSamplesPoint, nPointBaseLine[j],sigPolarity[j]);
      wf->findPar(chargeWinL[j],chargeWinR[j],sigConstTh[j]);
