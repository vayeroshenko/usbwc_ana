...
void convBunchDataUSB2rootBin(TString inputDataFileList, TString inputDataFileFolder, TString outputrootFile){
  ...
  while (indataList >> mot ){
    ...
    fread (buffer,1,301,pFile);
    currentPositionByte = 301;
    //buffTmp = buffer[266];
    //for(i = 0;i<268;i++){
    ...
    if(buffer[245] != nChannels_str){
      cout<<" ERROR --> Wrong number of channles"<<endl
	  <<"           buffer[245]  != nChannels_str"<<endl
	  <<"           buffer[245]   = "<<buffer[245]<<endl
	  <<"           nChannels_str = "<<nChannels_str<<endl;
      assert(0);
    }
    nSamplingPoints_chek  = buffer[201];//i = 167   [
    nSamplingPoints_chek += buffer[202];//i = 168   1
    nSamplingPoints_chek += buffer[203];//i = 169   0
    nSamplingPoints_chek += buffer[204];//i = 170   2
    nSamplingPoints_chek += buffer[205];//i = 171   4
    nSamplingPoints_chek += buffer[206];//i = 172   ]
